window.addEventListener('DOMContentLoaded', () => {
    ...
    // câmera em pixels (posição do canto superior esquerdo do mapa)
    ...
    // seed determinística (troque por um número fixo para mapas repetíveis)
    ...
    for (let y = 0; y < mapHeight; y++) {
        ...
        for (let x = 0; x < mapWidth; x++) {
            ...
            else if (h < 0.33) row.push(5); // praia (areia)
    ...
    const tileFiles = {
        0: 'grass.png',    // grama
        1: 'forest.png',   // floresta
        2: 'mountain.png', // montanha
        3: 'ocean.png',    // oceano / água
        4: 'river.png',    // rio
        5: 'sand.png'      // praia / areia
    };
    tileFiles['dirt'] = 'dirt.png';
    ...
    function drawMap() {
        ...
        // desliga image smoothing para preservar pixel-art (se tiles forem pixel)
        ...
        for (let y = startY; y < endY; y++) {
            ...
            for (let x = startX; x < endX; x++) {
                ...
                // tenta desenhar imagem do tile
                const img = tileImages[terrain];
                if (img && img.complete && img.naturalWidth) {
                    try {
                        ctx.drawImage(img, drawX, drawY, drawW, drawH);
                    } catch (e) {
                        // fallback de cor se qualquer erro ocorrer
                        fillTileFallback(ctx, terrain, drawX, drawY, drawW, drawH, x, y);
                    }
                } else {
                    // fallback: mantém as cores originais (ou usa heightMap para montanha)
                    fillTileFallback(ctx, terrain, drawX, drawY, drawW, drawH, x, y);
                }
            }
        }
        ...
        // restaura smoothing padrão (opcional)
        ctx.imageSmoothingEnabled = true;
    }

    // fallback de cor (usa também heightMap para tons de montanha)
    ...
    function drawMinimap() {
        ...
        // tiles do minimapa (usamos cores para performance)
        for (let y = 0; y < mapHeight; y++) {
            for (let x = 0; x < mapWidth; x++) {
                const t = map[y][x];
                switch (t) {
